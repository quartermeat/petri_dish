...
- **Do not edit source code.** That means no changes to `.go`, `.java`, `.xml`, `.gradle`, `.sh`, `go.mod`, `go.sum`, or anything under `android/`, `core/`, `petridish/`, `mobile/`, `scripts/`, or `main.go`.
...
- Active Codex workspace: `/mnt/d/Codex/Projects/work/petri_dish`.
- Upstream fork source: `/home/jerem/work/hex_globe/CODEX_hex_globe`.
...
| `mobile/` | gomobile bridge (`mobile.SetGame(petridish.NewGame())`). |
| `main.go` | `!android` desktop entry. |
| `android/` | Gradle wrapper app consuming the generated `PetriDish.aar`. |
...
- `petridish/game.go:drawGlobe` — painter's-algorithm renderer. Back-face cull, depth sort, pinhole project, triangle fan fill, stroked edges.
- `petridish/game.go:handlePointerInput` — unified mouse + touch drag-to-rotate.
...
- **Ruleset is the extension seam.** New game ideas belong in a new `core.Ruleset` impl, not in `petridish/` or `core/globe.go`.
- **Android package is `com.quartermeat.petridish`.** portrait-locked, minSdk 26, no release signing yet.
